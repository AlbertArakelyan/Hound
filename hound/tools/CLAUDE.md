...
- It imports no PyQt6. Page imports go inside the `_*_page()` factory functions, so
  opening the app does not import every page.
- A `ToolSpec` carries `id`, `name`, `description`, and `create_page`. The `name` and
  `description` are shown on the home screen card, so write them for a person.
- Adding a tool means one new factory plus one new `TOOLS` entry. Nothing else.

## Inside a tool

- Logic files import no PyQt6, so they can be tested with no display and no network.
